...
Earl Grey v7.2.6 patches the Dfam 3.9 download URL in the installation helper script generated by all three pipelines (`earlGrey`, `earlGreyLibConstruct`, `earlGreyAnnotationOnly`):

- **Dfam 3.9 download URL fixed** (`earlGrey`, `earlGreyLibConstruct`, `earlGreyAnnotationOnly`): the URL embedded in the generated `configure_dfam39.sh` helper script previously pointed to `https://dfam.org/releases/current/families/FamDB/`. The `current` symlink on the Dfam server now resolves to a later release, causing the generated `curl` command to download the wrong database version. Both the warning message and the generated `curl` command now point to the explicit versioned path `https://dfam.org/releases/Dfam_3.9/families/FamDB/`, ensuring correct and reproducible downloads.
